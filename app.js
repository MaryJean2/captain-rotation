...
(() => {
  ...
  const STORAGE_KEY = 'captainsRotation.v1';
  const CONTRACTED_DAYS = 185;
  ...
  const monthsCollection = collection(db, 'captainsRotation', 'shared', 'months');
  const configRef = doc(db, 'captainsRotation', 'shared');
  ...
  function updateSelectionMessage() {
    ...
    if (!selectionEnd) {
      ...
      msg.textContent = `${formatDM(a)} → ${formatDM(b)} selected — choose David, Ewan, H or Clear.`;
    }
  }
  ...
  function labelForCode(code) {
    return code === 'D' ? 'David' : code === 'E' ? 'Ewan' : code === 'H' ? 'Handover' : '';
  }

  function getRotations() {
    const keys = Object.keys(state.days).sort();
    if (!keys.length) return [];
    const workKeys = keys.filter(k => state.days[k] === 'D' || state.days[k] === 'E');
  ...
  function renderRotations() {
    ...
    for (const r of rotations) {
      ...
      row.innerHTML = `
        <strong><i class="person-dot ${r.code}"></i>${r.code === 'D' ? 'David' : 'Ewan'}</strong>
  ...
  function contractWindow(person, startYear) {
    const name = person === 'D' ? 'David' : 'Ewan';
    return {
      start: new Date(startYear, 10, 1, 12),
      end: new Date(startYear + 1, 9, 31, 12),
      label: `${name} ${formatDM(new Date(startYear, 10, 1, 12))} ${startYear} → ${formatDM(new Date(startYear + 1, 9, 31, 12))} ${startYear + 1}`
    };
  }
  ...
  function renderSummary() {
    ...
    contractBody.innerHTML = contracts.map((c, i) => {
      ...
      return `<tr>
        <th><strong>${i === 0 ? 'David' : 'Ewan'}</strong><span>${c.label.replace(i === 0 ? 'David ' : 'Ewan ','')}</span></th>
  ...
  function exportBackup() {
    const payload = { app: 'Captains Rotation Planner', version: 1, exportedAt: new Date().toISOString(), days: state.days };
  ...
  async function replaceCloudPlan(days) {
    ...
    for (const [key, code] of Object.entries(days)) {
      if (!['D','E','H'].includes(code)) continue;
      const monthId = monthDocIdFromKey(key);
      grouped[monthId] ||= {};
    ...
    reader.onload = async () => {
      try {
        ...
        for (const [key, value] of Object.entries(days)) {
          if (/^\d{4}-\d{2}-\d{2}$/.test(key) && ['D','E','H'].includes(value)) cleaned[key] = value;
        }
        if (!confirm('Replace the shared planner for both David and Ewan with this backup?')) return;
  ...
  function exportCsv() {
    ...
    for (const r of rotations) rows.push([r.code === 'D' ? 'David' : 'Ewan', formatDM(r.displayStart), formatDM(r.displayEnd), formatDuration(r.days), r.days]);
  ...
  function monthSnapshotToDays(snapshot) {
    ...
    for (const monthDoc of snapshot.docs) {
      ...
      for (let d = 1; d <= 31; d++) {
        ...
        if (['D','E','H'].includes(data[field])) days[`${monthId}-${String(d).padStart(2, '0')}`] = data[field];
      }
    }
    return days;
  }
  ...
  $('clearAll').addEventListener('click', async () => {
    if (!currentUser) return;
    if (!confirm('Delete ALL shared rotation data for both David and Ewan?')) return;
